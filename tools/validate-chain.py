import json, time, urllib.request, urllib.error, secrets, string, sys, os, glob
...
H = {"Authorization": f"Bearer {PAT}", "Content-Type": "application/json",
     "User-Agent": "deedbox-schema-validator/1.0"}
...
proj = req("POST", "/projects", {
    "name": "deedbox-scratch-0001", "organization_id": org,
...
def run_sql_file(path, label):
    ...
    body = {"query": q}
    out = req("POST", f"/projects/{ref}/database/query", body)
    print(f"{label}: OK -> {json.dumps(out)[:200]}")

try:
    # The chain and its suites are enumerated from THIS repository checkout
    # (worktree-safe), sorted by number: apply every change, then run every
    # suite. No hand-maintained list — a new numbered pair is picked up
    # automatically, and a change file without a suite fails loudly.
    repo = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    changes = sorted(glob.glob(os.path.join(repo, "schema", "changes", "*.sql")))
    suites = sorted(glob.glob(os.path.join(repo, "schema", "tests", "*.test.sql")))
    if not changes:
        raise SystemExit("no schema change files found")
    if len(suites) != len(changes):
        raise SystemExit(
            f"{len(changes)} change files but {len(suites)} suites — every change ships its suite")
    for path in changes:
        run_sql_file(path, f"APPLY {os.path.basename(path)[:4]}")
    for path in suites:
        run_sql_file(path, f"TESTS {os.path.basename(path)[:4]}")
...
# NOTE: change files + suites are discovered by glob from this checkout —
# nothing to extend by hand. Creates a scratch Supabase project (Mgmt API,
# custom User-Agent required or Cloudflare 403s), applies the full chain, runs
# every suite, deletes the scratch. Any raised exception fails the run.
